...
public class MMU {
        ...
        if (!Memory.SEGMENT) {
            ...
            if (!Memory.PAGE) {
                ...
                for (int i = startvPageNo; i <= endvPageNo; i++) {
                    if (TLB.isAvailable) {
                        // TODO: add tlb here
                        if (!tlb.isValidPage(i)) { // 不在TLB中
                            if (!memory.isValidPage(i)) {
                                // 缺页中断，该页不在内存中，内存从磁盘加载该页的数据
                                memory.page_load(i);
                            }
                            else {
                                // 该页不在TLB但在页表中，将该虚页项填到TLB中
                                tlb.write(i);
                            }
                        }
                    } else {
    ...
    private String toRealLinearAddr(String logicAddr) {
        ...
        int addressBase = Integer.parseInt(transformer.binaryToInt("0" + logicAddr.substring(0,16))) * 16;
        int offset = Integer.parseInt(transformer.binaryToInt("0" + logicAddr.substring(16)));
        String linearAddress = transformer.intToBinary(addressBase + offset + "");
        while (linearAddress.length() < 32) linearAddress = "0" + linearAddress;
    ...
    private String toSegLinearAddr(String logicAddr) {
        // TODO: paste your code here
        String index = logicAddr.substring(0,13);
        int segIndex = Integer.parseInt(transformer.binaryToInt("0" + index));
        String addressBase = String.valueOf(memory.getBaseOfSegDes(segIndex));
        String offset = logicAddr.substring(16);
        while (addressBase.length() < 32) addressBase = "0" + addressBase;
        String linearAddress = alu.add(new DataType(addressBase), new DataType(offset)).toString();
        return linearAddress;
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        if (TLB.isAvailable) {
            //访问TLB取物理页框号;
            int index = Integer.parseInt(transformer.binaryToInt(linearAddr.substring(0, 20))); //虚拟页号
            String pagePhysical = String.valueOf(tlb.getFrameOfPage(index)); // 得到物理页框号
            String physicalAddress = pagePhysical + linearAddr.substring(20);
            return physicalAddress;
        } else {
            //访问页表取物理页框号;
            int index = Integer.parseInt(transformer.binaryToInt(linearAddr.substring(0, 20))); //虚拟页号
            String pagePhysical = String.valueOf(memory.getFrameOfPage(index));
            String physicalAddress = pagePhysical + linearAddr.substring(20);
            return physicalAddress;
        }
    }
