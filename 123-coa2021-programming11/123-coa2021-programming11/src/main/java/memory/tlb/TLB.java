...
public class TLB {
    ...
    public boolean isValidPage (int vPageNo) {
        for (int i = 0; i < TLB_SIZE; i++) {
            // 从头到尾把TLB遍历一遍，查找TLB中存放的虚拟页号是否和想要的一样
            TLBLine tlbLine = getTLB().getTLBLine(i);
            if (tlbLine.vPageNo == vPageNo) return tlbLine.valid;
        }
        return false;
    }
    ...
    public char[] getFrameOfPage (int row) {
        for (int i = 0; i < TLB_SIZE; i++) {
            TLBLine tlbLine = getTLBLine(i);
            if (tlbLine.vPageNo == row) {
                return tlbLine.pageFrame;
            }
        }
        return null;
    }
