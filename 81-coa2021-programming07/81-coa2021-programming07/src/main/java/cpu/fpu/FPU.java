...
public class FPU {
    ...
    public DataType add(DataType src, DataType dest) {
        ...
        return new DataType(total.substring(1)); // 第一位来判断结果是否溢出
    }
    ...
    private class FPUHelper {
        ...
        public String floatAddition(String operand1, String operand2, int eLength, int sLength, int gLength) {
            ...
            // 因为非规格化数表示范围最大到2^-126次方，转化成规格化数便是exp = 1，偏移量为127
            ...
            // 判断有效值部分是否因为右移全部变成0，若为0，则返回0加上另一个有效值
            // 这里0代表没有溢出
            ...
            if (overflow) {
                ...
                if (exponent == maxValue(eLength)) {  //指数上溢
                    answer.append("1").append(sign); // 这里的1表示溢出
        ...
        public String floatMultiplication(String operand1, String operand2) {
            ...
            if (exponent >= bias * 2 + 1) {  //infinite 此时阶码上溢
                answer.append("1"); // 这个1表示有溢出
                answer.append(sign);
        ...
        public String floatDivision(String operand1, String operand2) {
            ...
            int exponent = Xexponent - Yexponent + bias;
            String temp = unsignedDivision(Xsig, Ysig);  // 无符号数的除法 27位
            StringBuilder answer = new StringBuilder();
            //特殊除法，第一位就是隐藏位？？
            while (temp.charAt(0) == '0' && exponent > 0) {
                temp = leftShift(temp, "1");
         ...
         * @param eLength exponent's length 8
         * @param sLength significand's length 23
