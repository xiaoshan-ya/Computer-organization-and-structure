...
public class Memory {
    ...
    public void seg_load(int segIndex) {
        // TODO
        SegDescriptor segDescriptor = getSegDescriptor(segIndex); // 通过段描述符索引得到对应的段描述符
        if (!segDescriptor.validBit) {
            char[] base = new char[32];
            char[] limit = new char[20];
            Arrays.fill(base,'0');
            Arrays.fill(limit, '1');
            segDescriptor.base =base;
            segDescriptor.limit = limit;
            segDescriptor.validBit = true;

            String pAddr = String.valueOf(segDescriptor.base);
            int len = Integer.parseInt(transformer.binaryToInt("0" + String.valueOf(segDescriptor.limit)));
            char[] data = disk.read(pAddr,len);
            if (!Memory.PAGE) write("00000000000000000000000000000000",len,data);
            if (!Memory.PAGE) segDescriptor.granularity = false;
            else segDescriptor.granularity = true;
        }
    }
    ...
    public void page_load(int vPageNo) {
        ...
        String pAddr = transformer.intToBinary(vPageNo + "");
        // 虚拟页号后补12个0，即偏移量为0
        pAddr = alu.mul(new DataType(pAddr), new DataType(transformer.intToBinary("" + PAGE_SIZE_B))).toString();
        ...
        for (int i = 0;;i++) {
            if (!pageValid[i]) {
                pageValid[i] = true;
                // 物理页框数 = 虚拟页数，也就是都是20位
                pageItem.pageFrame = transformer.intToBinary("" + i).substring(12).toCharArray();
                String inputAddress = alu.mul(new DataType(transformer.intToBinary("" + i)), new DataType(transformer.intToBinary("" + PAGE_SIZE_B))).toString();
                write(inputAddress,PAGE_SIZE_B,data);
                break;
            }
        }
    }
