...
public class Transformer {
    ...
    public String floatToBinary(String floatStr) {
        ...
        // 规格化数的阶的范围是[-126, 127] 因为规格化数阶码非全0或全1，即从0000 0001 ~ 1111 1110,真实值=阶码-127
        for (exp = -126; ; exp++) {
            if (Math.pow(2, exp) <= Float.parseFloat(abs) && Float.parseFloat(abs) < Math.pow(2, exp + 1)) {
                break;
            }
            // exp == 128表示该数不在规格化数表示的范围内，所以应在非规格化数的范围内
            if (exp == 128) { //
                exp = -126;
                isNormalized = false;
                break;
        ...
        if (!isNormalized) { // 如果是非规格化数
            expStr = "00000000";
            // 算有效位 浮点数表示法:S * 2^E = abs
            // 小数转二进制数:将该数字乘以2，取出整数部分作为二进制表示的第1位；然后再将小数部分乘以2，将得到的整数部分作为二进制表示的第2位；以此类推，直到小数部分为0
